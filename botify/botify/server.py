...
from botify.recommenders.IndexedModified import IndexedModified
...
recommendations_contextual = Redis(
    app, config_prefix="REDIS_RECOMMENDATIONS_CONTEXTUAL")
recommendations_gcf = Redis(app, config_prefix="REDIS_RECOMMENDATIONS_GCF")
recommendations_div = Redis(
    app, config_prefix="REDIS_TRACKS_WITH_DIVERSE_RECS")
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        #     recommender = TopPop(
        #         catalog.top_tracks[:100], Random(tracks_redis.connection))
        # elif treatment == Treatment.T3:
        #     recommender = Indexed(
        #         recommendations_lfm.connection, catalog, Random(tracks_redis.connection))
        # elif treatment == Treatment.T4:
        #     recommender = Indexed(
        #         recommendations_dssm.connection, catalog, Random(tracks_redis.connection))
        # elif treatment == Treatment.T5:
        #     recommender = Contextual(
        #         recommendations_contextual.connection, catalog, Random(tracks_redis.connection))
        # elif treatment == Treatment.T6:
        #     recommender = Contextual(
        #         recommendations_div.connection, catalog, Random(tracks_redis.connection))
        # else:
        #     recommender = Random(tracks_redis.connection)
