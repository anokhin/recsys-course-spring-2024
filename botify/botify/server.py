...
from botify.recommenders.ImprovedIndexed import ImprovedIndexed
...
recommendations_dssm_improved = Redis(app, config_prefix="REDIS_RECOMMENDATIONS_DSSM_IMPROVED")
...
catalog.upload_recommendations(
    recommendations_dssm_improved.connection, "RECOMMENDATIONS_DSSM_IMPROVED_FILE_PATH"
)
catalog.upload_recommendations(
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        if treatment == Treatment.T1:
            recommender = ImprovedIndexed(
                recommendations_dssm.connection, recommendations_lfm.connection, catalog, 
                Indexed(recommendations_dssm.connection, catalog, Random(tracks_redis.connection))
            )
        elif treatment == Treatment.T2:
            recommender = ImprovedIndexed(
                recommendations_dssm_improved.connection, recommendations_lfm.connection, catalog, 
                Indexed(recommendations_dssm.connection, catalog, Random(tracks_redis.connection))
            )
        else:
