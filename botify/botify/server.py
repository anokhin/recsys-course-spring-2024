...
recommendations_my_rec = Redis(app, config_prefix="REDIS_RECOMMENDATIONS_MY_REC")
...
catalog.upload_recommendations(
    recommendations_my_rec.connection, "RECOMMENDATIONS_MY_REC_FILE_PATH"
)
catalog.upload_recommendations(
...
weighted_recs = {user : None for user in range(10000)} 
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        if treatment == Treatment.T1:
            recommender = MyIndexed(recommendations_dssm.connection, catalog, Random(tracks_redis.connection), weighted_recs)
