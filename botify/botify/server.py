...
recommendations_dssm_updated = Redis(app, config_prefix="REDIS_RECOMMENDATIONS_DSSM_UPDATED")
...
catalog.upload_recommendations(
    recommendations_dssm_updated.connection, "RECOMMENDATIONS_DSSM_UPDATED_FILE_PATH",
)
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        treatment = Experiments.DSSM_UPDATED.assign(user)

        if treatment == Treatment.T1:
            recommender = Indexed(recommendations_dssm_updated.connection, catalog, Random(tracks_redis.connection))
        else:
