...
from botify.recommenders.my import MyRecommender
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        treatment = Experiments.MY.assign(user)

        if treatment == Treatment.T1:
            recommender = MyRecommender(recommendations_div.connection, recommendations_dssm.connection, catalog, Random(tracks_redis.connection))
        else:
